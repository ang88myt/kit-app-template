...
class CustomBoolWidget(CustomBaseWidget):
    ...
    def __init__(self, model: ui.AbstractItemModel = None, default_value: bool = False, on_change_callback=None, **kwargs):
        self.__default_val = default_value
        self.__bool_image = None
        self.__on_change_callback = on_change_callback  # Store the callback
    ...
    def _on_value_changed(self):
        """Swap checkbox images and set revert_img to correct state. Execute the callback if provided."""
        self.__bool_image.checked = not self.__bool_image.checked
        self.__bool_image.name = "checked" if self.__bool_image.checked else "unchecked"
        self.revert_img.enabled = self.__default_val != self.__bool_image.checked
        if self.__on_change_callback:  # Check if there is a callback provided
            self.__on_change_callback(self.__bool_image.checked)  # Pass the new state to the callback
